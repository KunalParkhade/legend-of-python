#In programming, variables are used for storing data values. Each variable has a name and holds a value. 📦
#The variable name can consist of letters, numbers, and the _ underscore character.
#These are all valid variable names and values:
name = 'Kunal Parkhade'
user_id = 210387352
progress = 0.7
exp = 70
verified = True

#The = equal sign means assignment:
#We're assigning the string value 'Kunal Parkhade' to the variable name.
#We're assigning the number value 210387352 to the variable user_id.
#We're assigning the number value 0.7 to the variable progress.
#We're assigning the number value 70 to the variable exp.
#We're assigning the truth value True to the variable verified.
#We can also change the value of a variable, or print it out:
xp = 70
xp = 80

print(xp)    # Output: 80

#Here, we are assigning the number value 70 to the variable xp. Then, we are reassigning the integer value 80 to the same variable. And printing it out.

#An integer, or int, is a whole number. It has no decimal point and contains the number 0, positive and negative counting numbers.
year = 2024
age = 20

#A floating-point number, or a float, is a decimal number. It can be used to represent fractions or precise measurements.
pi = 3.14159
height = 1.75

#A string, or str, is used for storing text. Strings are wrapped in double quotes " " or single quotes ' '.
message = "Rock!!!"
user = '@kparkhade'

#A Boolean data type, or bool, stores a value that can only be either true or false. In Python, it's capitalized True or False.
bowled = True
not_bowled = False