...
#So far, the functions we've created don't take in any input(s), which means they do the same thing each time they get called. 
#A function can be far more useful than that!
#Sometimes, we want our functions to perform a specific task, but the task varies depending on different input(s). 
#And that's where parameters come in.

#Parameters are variables that a function takes in as input. 
...
#Suppose you're a NASA intern, and your team is calculating the distance a rocket needs to travel. 
#The value calculated is in kilometers, while the next step requires miles. Your manager quickly writes this on the board:
